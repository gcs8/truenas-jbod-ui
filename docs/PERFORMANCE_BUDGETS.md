...
Export measurements report the pre-inline HTML document bytes and the inlined static asset bytes separately, as well as the complete HTML and retained-cache totals. Normal `app.js`, `style.css`, or offline-image edits inside the bounded band do not require baseline regeneration. Growth beyond the band still fails. Inlined static assets also have an independent 2 MiB hard ceiling for both modeled scales. The reviewed baseline uses 550,407 of 2,097,152 bytes (26.25%), leaving 1,546,745 bytes (73.75%): the modeled fixture has no NVMe carrier or SATADOM view, so it inlines the stylesheet and script only. An export that can draw a card photo adds that photo as a data URL (the M.2 carrier board is the largest at about 970 KB), which still fits under the ceiling. The 2 MiB ceiling remains unchanged: current usage is not a rationale for weakening the signal. Bundle and load-path optimization remains scoped to #337, while the complete export remains subject to its separate hard ceiling.
