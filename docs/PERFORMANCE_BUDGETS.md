...
Export measurements report the pre-inline HTML document bytes and the inlined static asset bytes separately, as well as the complete HTML and retained-cache totals. Normal `app.js`, `style.css`, or offline-image edits inside the bounded band do not require baseline regeneration. Growth beyond the band still fails. Inlined static assets also have an independent 2 MiB hard ceiling for both modeled scales. The reviewed baseline uses 1,597,836 of 2,097,152 bytes (76.19%), leaving 499,316 bytes (23.81%). The 2 MiB ceiling remains unchanged: current usage is not a rationale for weakening the signal. Bundle and load-path optimization remains scoped to #337, while the complete export remains subject to its separate hard ceiling.
