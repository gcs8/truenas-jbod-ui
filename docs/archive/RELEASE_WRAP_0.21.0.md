...
| Python unit and syntax gates | yes | rebuilt local release branch: `.venv/bin/python -m compileall -q app admin_service history_service scripts tests` passed; final `.venv/bin/python -m unittest discover -s tests -p "test_*.py" -q` passed `512` tests with `4` skipped after the SAS path-nibble regression was added; earlier targeted `tests.test_release_status -v` passed `4` | Pass |  |
