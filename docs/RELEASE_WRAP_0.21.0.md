...
| Feature-specific live API/UI gates | yes | blocked after gcs8 human QA rejected the restored candidate: the dataset presented as sanitized/default data rather than real systems, so prior local/Linux feature probes cannot count as real-system validation. Rerun `/api/inventory`, `/api/storage-views`, `/api/sas-fabric`, representative `/` and `/sas-fabric` browser paths, first-click, overflow, and console checks only after restoring a real long-running deployment backup. | Blocked |  |
...
| Linux QA restore gate | yes | blocked after gcs8 human QA rejected the `10.13.37.138:18080/18081/18082` candidate because it did not contain real deployment systems/data and appeared to be a sanitized/default instance. The previous disposable restore evidence from bundle `artifacts/private-v0.21.0/windows-restore-default.tar.zst` is invalid for release acceptance. Next attempt must restore a real long-running deployment backup through the disposable Linux admin API, verify real-data provenance before calling the row `Pass`, and then get gcs8 acceptance. | Blocked |  |
| Restored Linux QA perf harnesses | yes | blocked because the only restored Linux QA perf evidence was gathered from the human-rejected sanitized/default candidate. Rerun main and history perf harnesses serially after a real-data Linux QA restore and after `:18081/healthz` reports `collection_running=false`. | Blocked |  |
| Snapshot/export/offline artifact gate | yes | blocked for release because the Linux QA snapshot/export/offline evidence was gathered from the human-rejected sanitized/default candidate. The local mechanics smoke remains useful as a code check, but release evidence must repeat estimate, forced ZIP download, and offline browser smoke against the next real-data Linux QA restore. | Blocked |  |
