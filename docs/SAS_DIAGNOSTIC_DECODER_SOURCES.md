...
| INCITS T10 SCSI Status Codes list | Standards-backed SCSI status names | Public numeric list at <https://www.t10.org/lists/2status.htm>. Use for high-confidence generic SCSI status rows such as CHECK CONDITION, BUSY, RESERVATION CONFLICT, TASK SET FULL, ACA ACTIVE, and TASK ABORTED. |
| INCITS T10 ASC/ASCQ list | Standards-backed SCSI sense reason names | Public numeric list at <https://www.t10.org/lists/asc-num.htm> (current public page reports a `2024-11-28` list date). Use for high-confidence ASC/ASCQ labels and fault families such as logical-unit communication, write/read errors, protocol CRC/parity errors, ACK/NAK timeout, NAK received, connection lost, data-buffer errors, PCIe fabric errors, recovered-data conditions, write-protect conditions, enclosure warnings, logical-unit timeout/failure, log exceptions, rebuild/recalculate failures, and failure-prediction indicators. |
| FreeBSD `intro(2)` errno list | CORE CAM/errno interpretation | Public manual page at <https://man.freebsd.org/cgi/man.cgi?apropos=0&manpath=freebsd&query=intro&sektion=2>. Use only to translate CORE CAM numeric errno values, currently `Error 5` to `EIO` / input-output error. Keep the event confidence `observed` because the kernel log line supplies the CAM context and this is OS evidence rather than a SCSI/T10 decode. |
...
| TrueNAS Community Dell R740 CAM/MPR thread | Real-world example corpus only | Public example at <https://www.truenas.com/community/threads/error-creating-pool-on-dell-r740-cam-status-ccb-request-completed-with-an-error.93633/> shows the field pattern `WRITE(10)` + CAM failure + `Error 5, Retries exhausted` + LSI `loginfo 3112010c/31120302`; the reporter later resolved it by replacing SAS cables. Use it to shape parser fixtures and operator wording, not as a standards source. |
...
- `observed`: the kernel, OS manual, or appliance supplied a plain-language
  label, but the value is not a SCSI/T10 or vendor-loginfo table decode.
...
- Keep the current MPR/CAM evidence decoder centered on generic facts first:
  ...
  list length, CAM status, CAM errno/retry markers, SCSI status, sense key,
  ASC/ASCQ, and plain-language fault family.
...
- Add lookup tables incrementally where the source is stable:
  ...
  names.
- Keep severity tied to decoded fault family when a standard ASC/ASCQ value is
  known. Standard warning/recovery conditions should stay warning-level, while
  SAS protocol/link-loss/timeout and read/write media failures should remain
  error-level.
...
- Full SCSI/SAS lookup coverage. Current code carries broader common CDB
  ...
  RECEIVE COPY RESULTS service actions, useful status codes, useful ASC/ASCQ
  labels across transport, media, recovery, warning, target-port state,
  path-loss, log-exception, state-change, and failure-prediction families, LOG
  SENSE page names/page-control fields, SAS PHY concepts, and SES/AES
  concepts; it is still curated rather than exhaustive.
