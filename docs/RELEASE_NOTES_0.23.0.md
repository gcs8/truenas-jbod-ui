...
- Hardening is opt-in through `docker-compose.nonroot.yml`. Adopting that overlay requires a one-time ownership step before first start: stop the stack and `sudo chown -R` the bind mounts (`./config`, `./data`, `./logs`, `./history`, `./backup-status`) to the configured `APP_UID` and `APP_GID`. The base Compose file is root-compatible and needs nothing. A repository checkout has a bounded preflight helper for the same step; see `scripts/README.md`.
