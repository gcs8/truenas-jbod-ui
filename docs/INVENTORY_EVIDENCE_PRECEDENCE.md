...
- Invalid AES descriptors carry no device slot number. They are mapped only when every valid AES descriptor proves one consistent element-to-device-slot offset and the translated empty bays do not collide; otherwise they remain unmapped element evidence instead of borrowing a neighbouring bay.
- The kernel enclosure-driver `slot` attribute is a device slot number. `enclosure_sysfs` device bindings are applied after the SES pages are merged and only to bays whose slot number came from an AES/join `device slot number`. Bays keyed by an EC element index, by translated invalid-descriptor evidence, or by `SlotNN` descriptor text take no sysfs hint, so an EC-only enclosure reports no device names rather than a neighbouring bay's disk; the dropped bindings surface as one warning per sg device ("Kernel enclosure bindings for /dev/sgN could not be placed: ..."). `SlotNN` text is excluded because no checked-in capture shows it equals the device slot number (the MD1280 join captures carry no `SlotNN` descriptors); a vendor whose `SlotNN` is shown to match could be admitted by adding `ses_description` to `SYSFS_JOINABLE_SLOT_NUMBER_SOURCES`.
