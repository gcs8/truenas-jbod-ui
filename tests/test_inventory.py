...
class InventoryStorageViewCandidateTests(unittest.TestCase):
    ...
    def test_get_storage_view_runtime_uses_saved_ses_profile_instead_of_live_selected_profile(self) -> None:
        ...
        self.assertEqual(saved_view.face_style, "front-drive")
        self.assertEqual(saved_view.latch_edge, "top")
        self.assertEqual(saved_view.bay_size, "2.5")
        self.assertEqual(saved_view.panel_title, "Front 24 Bay")
        self.assertEqual(saved_view.edge_label, "Front of chassis")
    ...
    # Issue #355: on CORE the multipath association used to come only from the
    # TrueNAS API disk record. When that record lacks `multipath_name` (disk
    # table not yet synced after a hot-added spare) and the passive member is
    # absent from the API disk list entirely, the parsed `gmultipath list`
    # consumers are the only evidence tying the bare member to its geom.
    CORE_GMULTIPATH_LIST_FIXTURE = """
Geom name: disk49
Type: AUTOMATIC
Mode: Active/Passive
...
""".strip()
    ...
    CORE_GLABEL_STATUS_FIXTURE = """
                                              Name  Status  Components
gptid/aaaa0001-0000-4000-8000-00000000c049     N/A  multipath/disk49p1
gptid/aaaa0002-0000-4000-8000-00000000c050     N/A  multipath/disk50p1
""".strip()

    CORE_CAMCONTROL_DEVLIST_FIXTURE = """
scbus12 on mpr0 bus 0:
<SYNTH MODEL 0001 R001>            at scbus12 target 153 lun 0 (da69,pass80)
<SYNTH MODEL 0001 R001>            at scbus12 target 154 lun 0 (da31,pass81)
scbus13 on mpr1 bus 0:
<SYNTH MODEL 0001 R001>            at scbus13 target 153 lun 0 (da84,pass95)
<SYNTH MODEL 0001 R001>            at scbus13 target 154 lun 0 (da70,pass96)
""".strip()

    CORE_ZPOOL_STATUS_GP_FIXTURE = """
  pool: synthpool
 state: ONLINE
config:

\tNAME                                            STATE     READ WRITE CKSUM
\tsynthpool                                       ONLINE       0     0     0
\t  12345678901234567890                          ONLINE       0     0     0
\t    /dev/gptid/aaaa0003-0000-4000-8000-00000000d001  ONLINE       0     0     0
\t    /dev/gptid/aaaa0004-0000-4000-8000-00000000d002  ONLINE       0     0     0
\tspares
\t  /dev/gptid/aaaa0001-0000-4000-8000-00000000c049  AVAIL
\t  /dev/gptid/aaaa0002-0000-4000-8000-00000000c050  AVAIL

errors: No known data errors
""".strip()

    CORE_SESUTIL_MAP_FIXTURE = """
ses2:
  Enclosure Name: ExampleCo SynthShelf
  Enclosure ID: 5000000000000101
  Element 1, Type: Array Device Slot
    Status: OK
    Description: Slot01
    Device Names: da84, pass95
  Element 2, Type: Array Device Slot
    Status: OK
    Description: Slot02
    Device Names: da70, pass96
""".strip()

    # Confirmed live shape: only the ACTIVE member is in the API disk list,
    # with empty multipath fields; the PASSIVE member (da69) is absent.
    CORE_UNSYNCED_MULTIPATH_API_DISK = {
        "name": "da84",
        "devname": "da84",
        "identifier": "{serial_lunid}SYNTH0000000049_5000c500a0000049",
        "serial": "SYNTH0000000049",
        "lunid": "5000c500a0000049",
        "model": "SYNTH MODEL 0001",
        "multipath_name": "",
        "multipath_member": "",
    }

    # The neighbouring spare whose API record does carry the multipath fields.
    CORE_API_NAMED_MULTIPATH_API_DISK = {
        "name": "da70",
        "devname": "multipath/disk50",
        "identifier": "{serial_lunid}SYNTH0000000050_5000c500a0000050",
        "serial": "SYNTH0000000050",
        "lunid": "5000c500a0000050",
        "model": "SYNTH MODEL 0001",
        "multipath_name": "disk50",
        "multipath_member": "da31",
    }

    def _build_core_multipath_service(self, temp_dir: str) -> InventoryService:
    ...
    def _parse_core_multipath_ssh_outputs(self) -> ParsedSSHData:
    ...
    def test_build_disk_records_backfills_multipath_from_gmultipath_consumers(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(record.multipath_name, "disk49")
            self.assertEqual(record.multipath_member, "da69")
            self.assertEqual(record.raw.get("multipath_source"), "gmultipath list")
            self.assertIn("multipath/disk49", record.lookup_keys)
            self.assertIn("gptid/aaaa0001-0000-4000-8000-00000000c049", record.lookup_keys)
    ...
    def test_build_disk_records_folds_second_unsynced_multipath_member_into_first_record(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            service = self._build_core_multipath_service(temp_dir)
            ssh_data = ParsedSSHData(
                glabel=parse_glabel_status(self.CORE_GLABEL_STATUS_FIXTURE),
            ...
            passive_member = dict(self.CORE_UNSYNCED_MULTIPATH_API_DISK, name="da69", devname="da69")
    ...
    def test_build_disk_records_keeps_api_named_multipath_over_gmultipath_backfill(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            baseline = service._build_disk_records(
                [dict(self.CORE_API_NAMED_MULTIPATH_API_DISK)],
                ParsedSSHData(glabel=parse_glabel_status(self.CORE_GLABEL_STATUS_FIXTURE)),
                {},
                {},
            )[0]
...
""".strip()
    ...
    def test_core_correlate_resolves_unsynced_multipath_spare_alongside_api_named_spare(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            service = self._build_core_multipath_service(temp_dir)
            ssh_data = self._parse_core_multipath_ssh_outputs()
            raw_data = TrueNASRawData(
                enclosures=[],
                disks=[
                    dict(self.CORE_UNSYNCED_MULTIPATH_API_DISK),
            ...
            slot_views, _enclosures, _selected_meta, _rows, _slot_count, _columns = service._correlate(
                raw_data,
                ssh_data,
                [],
            )

            by_device = {view.device_name: view for view in slot_views if view.device_name}
            self.assertIn("multipath/disk49", by_device)
            self.assertIn("multipath/disk50", by_device)
            backfilled = by_device["multipath/disk49"]
            api_named = by_device["multipath/disk50"]

            self.assertEqual(backfilled.gptid, "gptid/aaaa0001-0000-4000-8000-00000000c049")
            self.assertEqual(backfilled.pool_name, "synthpool")
            self.assertEqual(backfilled.vdev_class, "spare")
            self.assertEqual(backfilled.vdev_name, "spares")
            self.assertEqual(backfilled.topology_label, "synthpool > spares > spare")

            # Peer-highlight precondition (app.js sameStoragePeerGroup): both
            # spares carry the same pool_name and vdev_class "spare".
            self.assertEqual(api_named.pool_name, backfilled.pool_name)
            self.assertEqual(api_named.vdev_class, "spare")
            self.assertEqual(api_named.topology_label, backfilled.topology_label)

            self.assertIsNotNone(backfilled.multipath)
            self.assertEqual(backfilled.multipath.name, "disk49")
            self.assertEqual(backfilled.multipath.device_name, "multipath/disk49")
            self.assertEqual(backfilled.multipath.path_device_name, "da84")
            self.assertEqual(backfilled.multipath.alternate_path_device, "da69")
            self.assertEqual(backfilled.multipath.mode, "Active/Passive")
            self.assertEqual(
                [(member.device_name, member.state, member.controller_label) for member in backfilled.multipath.members],
                [("da69", "PASSIVE", "mpr0"), ("da84", "ACTIVE", "mpr1")],
            )
...
"""
    ...
    async def test_core_smart_summary_enriches_sparse_api_result_with_smartctl_text(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(summary.annualized_bytes_read, 255773160581149)
    ...
    async def test_core_ata_smart_summary_prefers_richer_ssh_json_when_available(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(summary.annualized_bytes_read, 273718477174)
    ...
    async def test_scale_smart_summary_falls_back_to_ssh_smartctl(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(summary.annualized_bytes_read, 58966883588835)
    ...
    async def test_linux_smart_summary_surfaces_nvme_wear_and_write_metrics(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(summary.bytes_read, 16925054629376000)
            self.assertEqual(summary.bytes_written, 2367983923200000)
            self.assertEqual(summary.annualized_bytes_read, 4592617741639060)
...
class InventorySlotDetailCacheTests(unittest.TestCase):
    ...
    def test_layout_unavailable_smart_withholds_the_departed_disk_during_the_unknown_window(
        self,
    ) -> None:
        # #525, layout-less reader: the last-good SMART fallback is keyed on the
        # bay alone, so during an identity-unknown window it served the departed
        # disk's persisted SMART fields (logical unit id, sas_address, power-on
        # hours, firmware) as the bay's last-good data. A bay-scoped sas_address
        # is not identity, so nothing cached may be served for the bay until a
        # strong identifier returns; when it does, the genuine carry-forward
        # resumes.
        with tempfile.TemporaryDirectory() as temp_dir:
            service = build_inventory_service(
                Settings(),
            ...
            # The swap window: SES still reports the bay, the API disk record
            # carries no strong identifier yet.
            degraded = slot_view(sas_address="sas-invented-005")
            asyncio.run(service._apply_and_persist_snapshot_slot_details([degraded]))
            self.assertEqual(degraded.identity_state, "unknown")
            ...
            # The entry survives as historical evidence; it is only withheld.
            self.assertEqual(
                service.slot_detail_store.get_entry("default", "enc-1", 5)
                .smart_fields.get("power_on_hours"),
                321,
            )

            # The same disk comes back: the genuine carry-forward resumes.
            returned = slot_view(
                serial="SER-VERIFY-005", sas_address="sas-invented-005",
            )
            asyncio.run(service._apply_and_persist_snapshot_slot_details([returned]))
            self.assertEqual(returned.identity_state, "known")
            resumed = service.get_cached_slot_smart_summary_without_layout(5, "enc-1")
            self.assertIsNotNone(resumed)
            assert resumed is not None
            self.assertEqual(resumed.power_on_hours, 321)

    def test_layout_unavailable_smart_withholds_a_replacement_disks_bay(self) -> None:
        # #525: after a replacement serial arrives the bay is identity-known
        # again, but the persisted SMART on record still belongs to the disk
        # that left, so the layout-less fallback must not serve it for the new
        # occupant either.
        with tempfile.TemporaryDirectory() as temp_dir:
            service = build_inventory_service(
                Settings(),
...
class InventoryServiceMutationRefreshTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_background_stale_snapshot_refresh_recollects_sources_without_invalidating_sg_cache(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIs(first, stale_snapshot)
            self.assertIs(second, stale_snapshot)
    ...
    async def test_background_stale_snapshot_views_share_one_source_refresh(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            def make_snapshot(enclosure_id: str) -> InventorySnapshot:
                return InventorySnapshot(
                    slots=[],
                    refresh_interval_seconds=30,
                    selected_system_id=system.id,
            ...
            stale_snapshots = {
                enclosure_id: make_snapshot(enclosure_id)
            ...
            expired = datetime.now(timezone.utc) - timedelta(seconds=1)
            for enclosure_id, stale_snapshot in stale_snapshots.items():
                service._cache[enclosure_id] = stale_snapshot
                service._cache_until[enclosure_id] = expired
            service._source_bundle = self._empty_source_bundle(warning="stale source bundle")
            service._source_bundle_until = datetime.now(timezone.utc) + timedelta(minutes=5)
            refreshed_bundle = self._empty_source_bundle(warning="refreshed source bundle")
            collect_started = asyncio.Event()
            release_collect = asyncio.Event()
            ...
            async def build_snapshot(
                ...
                *,
                ...
                self.assertFalse(force_source_refresh)
                self.assertIs(service._source_bundle, refreshed_bundle)
                assert selected_enclosure_id is not None
                return refreshed_snapshots[selected_enclosure_id]

            service._collect_inventory_source_bundle = AsyncMock(side_effect=collect_source_bundle)
            service._build_snapshot = AsyncMock(side_effect=build_snapshot)

            stale_a = await service.get_snapshot(selected_enclosure_id="enc-a", allow_stale_cache=True)
            await asyncio.wait_for(collect_started.wait(), timeout=0.5)
            stale_b = await service.get_snapshot(selected_enclosure_id="enc-b", allow_stale_cache=True)
            tasks = tuple(service._snapshot_refresh_tasks[enclosure_id] for enclosure_id in ("enc-a", "enc-b"))
            release_collect.set()
            await asyncio.wait_for(asyncio.gather(*tasks), timeout=0.5)

            self.assertIs(stale_a, stale_snapshots["enc-a"])
            self.assertIs(stale_b, stale_snapshots["enc-b"])
            service._collect_inventory_source_bundle.assert_awaited_once_with()
            self.assertEqual(service._build_snapshot.await_count, 2)
            self.assertIs(service._cache["enc-a"], refreshed_snapshots["enc-a"])
            self.assertIs(service._cache["enc-b"], refreshed_snapshots["enc-b"])
    ...
    async def test_explicit_snapshot_force_refresh_clears_sg_ses_device_cache(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            settings = Settings()
            system = SystemConfig(id="offsite-scale", truenas=TrueNASConfig(platform="scale"))
            service = build_inventory_service(
            ...
            service._collect_inventory_source_bundle = AsyncMock(
                return_value=self._empty_source_bundle()
            )
    ...
    async def test_background_stale_source_bundle_refresh_preserves_valid_sg_ses_device_cache(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            settings = Settings()
            system = SystemConfig(id="offsite-scale", truenas=TrueNASConfig(platform="scale"))
            service = build_inventory_service(
            ...
            first = await service._get_inventory_source_bundle(allow_stale_cache=True)
            await asyncio.wait_for(collect_started.wait(), timeout=0.1)
            background_task = service._source_bundle_refresh_task
            second = await service._get_inventory_source_bundle(allow_stale_cache=True)
            self.assertIs(service._source_bundle_refresh_task, background_task)
            release_collect.set()
            assert background_task is not None
            await asyncio.wait_for(background_task, timeout=0.1)

            self.assertIs(first, stale_bundle)
            self.assertIs(second, stale_bundle)
            service._collect_inventory_source_bundle.assert_awaited_once_with()
            self.assertIs(service._source_bundle, refreshed_bundle)
            self.assertEqual(service._sg_ses_device_cache["10.0.0.10"], valid_sg_cache)
...
class ReviewRegressionTests(unittest.TestCase):
    ...
    def test_quantastor_unscoped_disk_uses_identity_without_shelf_slot_fallback(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            settings = Settings()
            system = SystemConfig(
                id="unscoped-disk-quantastor",
                default_profile_id="supermicro-ssg-2028r-shared-front-24",
                truenas=TrueNASConfig(platform="quantastor"),
            )
            service = build_inventory_service(settings, system, MagicMock(), MagicMock(), temp_dir)
                ...
                disks=[
                    {
                        ...
                        "storageSystemId": "node-a",
                        "devicePath": "/dev/sda",
                        "serialNumber": "UNSCOPED-DISK",
                        "scsiId": "5000000000000001",
                        "slot": 1,
                    },
                    {
                        "id": "disk-other-shelf",
                        ...
                        "enclosureId": "enc-b",
                        "devicePath": "/dev/sdz",
                        "serialNumber": "OTHER-SHELF-DISK",
                        "scsiId": "5000000000000002",
                        "slot": 0,
                    },
                    {
                        "id": "disk-unscoped-manual",
                        "storageSystemId": "node-a",
                        "devicePath": "/dev/sdb",
                        "serialNumber": "MANUAL-DISK",
                        "slot": 3,
                    },
                ],
                pools=[],
            ...
            records = service._build_quantastor_disk_records(raw_data, "node-a", "enc-a")

            self.assertEqual(
                [record.raw["id"] for record in records],
                ["disk-unscoped", "disk-unscoped-manual"],
            )
            ...
            service.mapping_store.get_mapping = MagicMock(  # type: ignore[method-assign]
                side_effect=lambda _system_id, _enclosure_id, slot, **_kwargs: (
                    manual_mapping if slot == 2 else None
                )
            )
            ...
            self.assertEqual(slots[0].device_name, "sda")
            self.assertEqual(slots[0].raw_status["mapping_resolution_source"], "sas-address")
            self.assertIsNone(slots[1].device_name)
            self.assertEqual(slots[2].device_name, "sdb")
            self.assertEqual(slots[2].raw_status["mapping_resolution_source"], "manual")
            self.assertIsNone(slots[3].device_name)
    ...
    def test_quantastor_duplicate_system_rows_are_deduplicated_before_bounded_shelf_expansion(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            settings = Settings()
            system = SystemConfig(
                id="bounded-options-quantastor",
                default_profile_id="supermicro-ssg-2028r-shared-front-24",
                truenas=TrueNASConfig(platform="quantastor"),
            )
            service = build_inventory_service(settings, system, MagicMock(), MagicMock(), temp_dir)
            raw_data = TrueNASRawData(
                ...
                systems=[
                    {"id": "node-a", "name": "Node A"},
                    {"id": "node-a", "name": "Node A duplicate"},
                    {"id": "node-b", "name": "Node B"},
                ],
                disks=[],
                pools=[],
                ...
                hw_enclosures=[
                    *[
                        {"id": f"enc-a-{index:02d}", "storageSystemId": "node-a"}
                        for index in range(40)
                    ],
                    *[
                        {"id": f"enc-b-{index:02d}", "storageSystemId": "node-b"}
                        for index in range(40)
                    ],
                ],
                disk_temperatures={},
            ...
            with patch("app.services.inventory.EnclosureOption", wraps=EnclosureOption) as option_factory:
                options = service._build_quantastor_enclosure_options(raw_data)

            self.assertEqual(
                [option.id for option in options],
                [
                    *[f"enc-a-{index:02d}" for index in range(40)],
                    *[f"enc-b-{index:02d}" for index in range(24)],
                ],
            )
            self.assertEqual(option_factory.call_count, 64)
