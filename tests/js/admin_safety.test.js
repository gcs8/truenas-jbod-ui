...
test("busy backup and debug exports coalesce and stay disabled through control sync", async () => {
  for (const operation of ["exportBackup", "exportDebugBundle"]) {
    ...
    const api = load([operation, "syncBackupControls", "fetchWithTimeout", "requestTimeoutError", "readDownloadOrError", ...(source.includes(`function run${operation[0].toUpperCase() + operation.slice(1)}(`) ? [`run${operation[0].toUpperCase() + operation.slice(1)}`, "runBackupOperation"] : [])], bindings);
