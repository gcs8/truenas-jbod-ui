...
test("offline Storage Fabric link remains an in-page target", () => {
  const sasFabricViewLink = {};
  const updateSasFabricViewLink = loadFunction("updateSasFabricViewLink", {
    bootstrap: { sasFabricViewUrl: "#sas-fabric-panel" },
    state: { snapshotMode: true },
    sasFabricViewLink,
    buildScopedUrl: () => {
      throw new Error("offline target must not become a scoped server URL");
    },
  });

  updateSasFabricViewLink();

  assert.equal(sasFabricViewLink.href, "#sas-fabric-panel");
});

test("offline Storage Fabric action opens and scrolls to the in-page panel", () => {
  const state = { snapshotMode: true, sasFabric: { open: false } };
  const events = [];
  const openSasFabricPanel = loadFunction("openSasFabricPanel", {
    state,
    renderAll() { events.push("render"); },
    sasFabricPanel: { scrollIntoView(options) { events.push(["scroll", options]); } },
    fetchSasFabric() { events.push("fetch"); },
  });
  const handleSasFabricViewLinkClick = loadFunction("handleSasFabricViewLinkClick", {
    state,
    openSasFabricPanel,
  });
  const event = { preventDefault() { events.push("prevent"); } };

  assert.equal(handleSasFabricViewLinkClick(event), true);
  assert.equal(state.sasFabric.open, true);
  assert.equal(events.length, 2);
  assert.equal(events[0], "render");
  assert.equal(events[1][0], "scroll");
  assert.equal(events[1][1].behavior, "smooth");
  assert.equal(events[1][1].block, "start");
});
