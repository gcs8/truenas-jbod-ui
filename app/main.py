...
# settings. The main UI swaps generations when config.yaml,
# runtime-overrides.yaml or profiles.yaml change; see app/settings_reload.py.
...
def get_snapshot_export_service() -> SnapshotExportService:
    def build(generation: SettingsGeneration) -> SnapshotExportService:
        configure_logging(generation.settings)
    ...
    return SETTINGS_RUNTIME.active().component("snapshot_export_service", build)


def after_config_reload(app: FastAPI, _before: Settings, after: Settings) -> None:
