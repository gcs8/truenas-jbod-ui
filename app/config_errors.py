...
class ConfigurationError(ValueError):
    """One plain sentence per configuration problem, raised in place of pydantic's report.

    A ``ValueError`` so every existing fail-closed caller and test that expects
    a rejected configuration keeps working; the pydantic chain is suppressed so
    the log ends on the sentences, not on a documentation link.
    """
