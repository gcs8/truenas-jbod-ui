...
<!--
Format (see CONTRIBUTING.md, "Changelog And Release Notes"):
- One line per merged pull request, past tense, ending with the pull request
  number as `(#N)`. Several numbers may share a line as `(#12, #13, and #14)`,
  and the reference may sit on a wrapped continuation line. Wrap at about 80
  columns.
- Entries live under the top `Unreleased` section in one of these subsections,
  in this order: Highlights, Breaking changes, Upgrade notes, Security, Added,
  Changed, Fixed, Performance, Docs, Internal. Empty subsections are omitted.
- Highlights hold three to five one-sentence release themes and are written
  at release time, not per pull request.
- A pull request labelled `breaking` must also add a bullet under Breaking
...
- The GitHub release body is generated by `gh release create --generate-notes`
  from the label categories in .github/release.yml, with this file's
...
- Backup policy: a `filesystem` target whose root is the local backup archive,
  ...
  mount) is now a configuration error. The backup scheduler will not start and
  the admin policy editor will not save it. Point the target at a separate
  directory or disk before upgrading. (#633)
...
- A `filesystem` backup target can no longer share a directory with the local
  ...
  retention pass, so a symlink or bind mount added later is also caught. (#633)
...
- Segmented history gained crash-safe later-generation rotation, recovery of
  interrupted restores, packaged lifecycle CLIs, and sealed segments the backup
  service can read without write access (#162, #234, #254, and #216).
...
- Legacy manual slot mappings saved by older releases under the unscoped
  ...
  enclosure, because on any larger deployment such a row cannot be tied to a
  specific shelf and could label or light up the wrong physical disk. On a
  multi-system or multi-shelf deployment those bays now render unmapped even
...
- `SCHEDULED_BACKUP_INCLUDED_GROUPS_JSON` order no longer looks like a scope
  ...
  to order. Status written before this change is migrated forward when it is
  still compatible with the current scope. When the scope genuinely changed,
  the old success evidence is cleared before the next attempt, so the first
  run after upgrading is treated as unauthorized for segmented retention until
  a new successful FULL backup completes (#214).
- Sealed history segments are now group-readable. The segment directory is
  published as `0750` and each immutable segment and `catalog.json` as `0640`,
  owned by the history identity and shared with the backup group, so the
  backup service can read them without write access. The publisher must own
  the hot history database. Do not run the generic ownership helper over an
  existing segmented-history tree. It applies shared-write `0770` and `0660`
  modes recursively, which would make sealed segments and private recovery
  artifacts writable by the backup group. Follow the bounded, quiesced repair
  procedure in
  [Backup, Restore, and Debug Bundles](wiki/Backup-Restore-and-Debug-Bundles.md#optional-scheduled-state-backups)
  before starting the history sidecar (#216).
- An enclosure-query failure now fails the whole API fetch. The TrueNAS
  websocket client used to swallow an enclosure-query error and return what
  looked like a successful but empty shelf. It now propagates the failure, so
  an API problem surfaces as a source error in the UI instead of a shelf that
  renders with no bays, and previously trusted cached enclosure topology is
  kept when a refreshed API source fails (#218).
- Pinned dependencies moved: `websockets` 17.0.1 to 17.1, `pydantic` 2.13.4 to
  2.13.5, `cryptography` 50.0.0 to 50.0.1, and the development-only
  `coverage[toml]` floor from 7.15.4 to 7.16.0. Rebuild or repull the image
  rather than upgrading dependencies in place (#123).
