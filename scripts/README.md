...
| `run_image_upgrade_smoke.py` | CI `Image-only upgrade smoke`: upgrades the previous public release by image pin only, then rolls back, on a disposable Linux Docker host with `sudo`. Longer report-only scenarios cover the non-root overlay (`--scenario hardened`), a kill inside each startup migration step (`--scenario interrupted-migration`) and exact segmented-catalog identity through upgrade and rollback (`--scenario segmented-catalog`). Synthetic state only. |
