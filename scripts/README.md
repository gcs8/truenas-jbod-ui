...
| `run_image_upgrade_smoke.py` | CI `Image-only upgrade smoke`: upgrades the previous public release by image pin only, then rolls back, on a disposable Linux Docker host with `sudo`. It reports the equal predecessor/candidate history schema as compatibility with no transition. Longer report-only scenarios cover the non-root overlay (`--scenario hardened`), a kill inside each schema-initialization step followed by an idempotent start (`--scenario interrupted-migration`) and exact segmented-catalog identity through upgrade and rollback (`--scenario segmented-catalog`). Synthetic state only. |
